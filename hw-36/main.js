...
var filmGenres = films.map(function (el) { return el.genre; }).flat();
var filterFilmGenres = new Set(filmGenres);
console.log(Array.from(filterFilmGenres));
//  2.  Собрать в массив всех актеров всех фильмов (без
// повторения) 
var actorsFilm = films.map(function (el) { return el.actors; }).flat();
var filterActorsFilm = new Set(actorsFilm);
console.log(Array.from(filterActorsFilm));
//  3.  Отсортировать фильмы по рейтингу по убыванию 
var sortFilmsByRating = films.map(function (el) { return el.imdbRating; }).sort(function (a, b) { return b - a; });
console.log(sortFilmsByRating);
var newFilmsArr = films.map(function (el) {
    return {
        id: el.id,
        title: el.title,
...
// выхода фильма. 
var numToYear = function (arr, num) {
    return arr.filter(function (el) { return el.year == num; });
};
...
// название фильма. 
var stringToFilm = function (arr, str) {
    return arr.filter(function (el) { return el.title.includes(str); });
};
...
var stringToFilmOrPlot = function (arr, str) {
    return arr.filter(function (el) { return el.title.includes(str) || el.plot.includes(str); });
};
console.log(stringToFilmOrPlot(films, "Luke"));
var filmFilter = function (arr, key, value) {
    return arr.filter(function (el) { return el[key] == value; });
};
